...
import { Eye, EyeOff, KeyRound, Lock, MessageCircle, Plus, Settings, Shield, Users } from "lucide-react";
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto animate-fade-in space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">Manage staff accounts, Telegram notifications, and your password</p>
      </div>
        ...
        <Card className="border shadow-sm">
          ...
          <CardContent className="space-y-4">
            <p className="text-xs text-muted-foreground">
              Each staff member with a Telegram Chat ID receives all reminder alerts automatically.
              Ask each nurse to send <code className="bg-muted px-1 rounded">/start</code> to <code className="bg-muted px-1 rounded">@userinfobot</code> on Telegram to get their ID.
            </p>
            {usersLoading ? (
            ...
            <div className="border-t pt-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input value={newStaff.name} onChange={(e) => setNewStaff(f => ({...f, name: e.target.value}))} placeholder="e.g., Nurse Aisha" className="rounded-lg h-9" />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs">Email *</Label>
                  <Input type="email" value={newStaff.email} onChange={(e) => setNewStaff(f => ({...f, email: e.target.value}))} placeholder="nurse@clinic.com" className="rounded-lg h-9" />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-xs">Password * (min 8 chars)</Label>
                  <div className="relative">
                    <Input type={showNewPwd ? "text" : "password"} value={newStaff.password} onChange={(e) => setNewStaff(f => ({...f, password: e.target.value}))} placeholder="Minimum 8 characters" className="rounded-lg h-9 pr-10" />
                ...
                <div className="space-y-1.5 col-span-2">
                  ...
                  <Input value={newStaff.telegramChatId} onChange={(e) => setNewStaff(f => ({...f, telegramChatId: e.target.value}))} placeholder="Ask them to send /start to @userinfobot and share their ID" className="rounded-lg h-9" />
                </div>
              </div>
              <Button onClick={() => {
                if (!newStaff.name.trim() || !newStaff.email.trim() || newStaff.password.length < 8) { toast.error("Name, email and a password of at least 8 characters are required"); return; }
                createStaff.mutate({ name: newStaff.name.trim(), email: newStaff.email.trim(), password: newStaff.password, role: newStaff.role, telegramChatId: newStaff.telegramChatId.trim() || undefined });
      ...
      <Card className="border shadow-sm">
        ...
        <CardContent className="space-y-3 max-w-sm">
          ...
          <div className="space-y-1.5">
            ...
            <Input type={showPwds ? "text" : "password"} value={pwdForm.next} onChange={(e) => setPwdForm(f => ({...f, next: e.target.value}))} placeholder="New password" className="rounded-lg h-9" />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs">Confirm New Password</Label>
            <Input type={showPwds ? "text" : "password"} value={pwdForm.confirm} onChange={(e) => setPwdForm(f => ({...f, confirm: e.target.value}))} placeholder="Repeat new password" className="rounded-lg h-9" />
          </div>
          <Button onClick={() => {
            ...
            if (pwdForm.next !== pwdForm.confirm) { toast.error("New passwords don't match"); return; }
